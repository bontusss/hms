...
from forms import PaymentForm, UserForm, LoginForm, RegistrationForm, AllocationForm, UnpaidForm
from models import Room, Student, Hostel
...
def landlord_login(request):
    ...
    if user is not None:
        try:
            if user.is_landlord and user.is_active:
                login(request, user)
                room_list = request.user.landlord.hostel.room_set.all()
...
def select(request):
    ...
    if request.method == 'POST':
        if not request.user.student.paid:
            return HttpResponse('You have dues. Please contact your Hostel Caretaker or Warden')
        form = AllocationForm(request.POST, instance=request.user.student)
        if form.is_valid():
        ...
        if not request.user.student.no_dues:
            return HttpResponse('You have dues. Please contact your Hostel Caretaker or Warden')
        form = AllocationForm(instance=request.user.student)
